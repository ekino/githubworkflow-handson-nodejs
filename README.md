...
- `docker/metadata-action@v5`: Extracts metadata from Git to create appropriate tags and labels for the Docker image:
  ...
  - `type=sha,format=long`: Tags the image with the full Git commit SHA
  - `type=ref,event=branch`: Tags the image with the branch name (e.g., `main`)
  - `type=ref,event=tag`: Tags the image with Git tags when they exist
...
- 🐳 **Docker Integration**: You've learned how to build and push Docker images as part of your CI/CD pipeline.
...
Once pushed, your image will be available at `ghcr.io/username/repo-name` and can be pulled with:

```bash
docker pull ghcr.io/username/repo-name:main
```
